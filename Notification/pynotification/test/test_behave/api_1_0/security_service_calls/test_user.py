...
@Given('sysadmin registered user "{user_name}" with a user_name and password - user_create')
...
@Given('sysadmin created user "{user_name}" with a user_name and password - user_create')
def step_impl(context, user_name):
    user_dict = {"user_name": "{}@gmail.com".format(user_name + str(ObjectId())), "password": str(ObjectId())}
    headers = {"token": context.users["sysadmin"]["token"]}
    result = requests.post(Url.SECURITY_DOMAIN_ADDRESS + Url.SECURITY_DOMAIN_USER_CREATE_ROUTE, json=user_dict, headers=headers)
...
@Given('sysadmin registered users in list with the user_name and passwords - user_create')
def step_impl(context):
    headers = {"token": context.users["sysadmin"]["token"]}
    for user in context.users:
        if user != "sysadmin":
            user_dict = context.users[user]
            user_dict["email"] += (str(ObjectId())) + "@mailinator.com"
            login_dict = {"user_name": user_dict["email"], "password": user_dict["password"]}
            result = requests.post(Url.SECURITY_DOMAIN_ADDRESS + Url.SECURITY_DOMAIN_USER_CREATE_ROUTE, json=login_dict, headers=headers)
            print(result)
...
@Given('users in the list are logged in with their user_name and password - login')
def step_impl(context):
    for user in context.users:
        user_dict = context.users[user]
        login_dict = {"user_name": user_dict["email"], "password": user_dict["password"]}
